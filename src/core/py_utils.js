...
import '@/libs/py.js/lib/py_extras.js';
import './pyeval';
...
function eval_domains(domains, evaluation_context) {
  ...
  _.each(domains, function (domain) {
    ...
    switch (domain.__ref) {
      case 'domain':
        evaluation_context.context = evaluation_context;
        domain_array_to_combine = py.eval(
          domain.__debug,
          wrap_context(evaluation_context)
        );
        break;
      default:
...
function pycontext() {
  return {
    ...
    current_date: py.PY_call(py.extras.time.strftime, [
      py.str.fromJSON('%Y-%m-%d'),
    ]),
    tz_offset: tz_offset,
  };
}
...
function py_eval(expr, context) {
  return py.eval(
    expr,
    _.extend({}, context || {}, { true: true, false: false, null: null })
  );
}
...
function assembleDomains(domains, operator) {
  ...
  var normalizedDomains = ASTs.filter(function (AST) {
...
function _formatAST(ast, lbp) {
  ...
  switch (ast.id) {
    ...
    case '=':
      return (
        _formatAST(ast.first) + ' ' + ast.id + ' ' + _formatAST(ast.second)
      );
    // operators
    case '-':
    case '+':
    ...
    case 'or':
      if (ast.second) {
        // infix
        var r =
          _formatAST(ast.first, ast.lbp) +
          ' ' +
          ast.id +
          ' ' +
          _formatAST(ast.second, ast.lbp);
        if (ast.lbp < lbp) {
          r = '(' + r + ')';
        }
        return r;
    ...
    case '(comparator)':
      ...
      return (
        _formatAST(ast.expressions[0]) +
        ' ' +
        operator +
        ' ' +
        _formatAST(ast.expressions[1])
      );
    ...
    case '(':
      if (ast.second) {
        ...
        return (
          _formatAST(ast.first) +
          '(' +
          ast.second.map(_formatAST).join(', ') +
          ')'
        );
      } else {
        // this is a tuple
...
function _normalizeDomainAST(domain) {
  ...
  if (expected < 0) {
    domain.first.unshift.apply(
      domain.first,
      _.times(Math.abs(expected), _.constant(andOperator))
    );
  }
