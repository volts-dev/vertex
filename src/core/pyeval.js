import 'underscore';
import '@/libs/py.js/lib/py.js';
//var py = window.py; // see vue.config.js
var pyeval = (window.pyeval = window.pyeval || {});
...
datetime.timedelta = py.type('timedelta', null, {
  __init__: function () {
    ...
    var daysecondsfrac = vectors.utils.modf(days, function (dayfrac, days) {
      d = days;
      if (dayfrac) {
        return vectors.utils.modf(dayfrac * 24 * 3600, function (dsf, dsw) {
          s = dsw;
          return dsf;
        });
      }
    ...
    var secondsfrac = vectors.utils.modf(seconds, function (sf, s) {
      seconds = s;
      return sf + daysecondsfrac;
    });
    vectors.utils.divmod(seconds, 24 * 3600, function (days, seconds) {
      d += days;
      s += seconds;
    });
    ...
    vectors.utils.divmod(microseconds, 1000000, function (
      seconds,
      microseconds
    ) {
      vectors.utils.divmod(seconds, 24 * 3600, function (days, seconds) {
  ...
  __str__: function () {
    var hh, mm, ss;
    vectors.utils.divmod(this.seconds, 60, function (m, s) {
      vectors.utils.divmod(m, 60, function (h, m) {
        hh = h;
        mm = m;
        ss = s;
      });
    });
    var s = vectors.utils.sprintf('%d:%02d:%02d', hh, mm, ss);
    if (this.days) {
      s = vectors.utils.sprintf(
        '%d day%s, %s',
        this.days,
    ...
    if (this.microseconds) {
      s = vectors.utils.sprintf('%s.%06d', s, this.microseconds);
    }
  ...
  strftime: function () {
    ...
    return py.str.fromJSON(
      args.format.toJSON().replace(/%([A-Za-z])/g, function (m, c) {
        switch (c) {
          case 'Y':
            return vectors.utils.sprintf('%04d', self.year);
          case 'm':
            return vectors.utils.sprintf('%02d', self.month);
          case 'd':
            return vectors.utils.sprintf('%02d', self.day);
          case 'H':
            return vectors.utils.sprintf('%02d', self.hour);
          case 'M':
            return vectors.utils.sprintf('%02d', self.minute);
          case 'S':
            return vectors.utils.sprintf('%02d', self.second);
        }
...
var relativedelta = py.type('relativedelta', null, {
  ...
  _fix: function () {
    ...
    if (Math.abs(months) > 11) {
      var s = months > 0 ? 1 : -1;
      var r = vectors.utils.divmod(months * s, 12);
      this.ops.months = py.float.fromJSON(r.mod * s);
      this.ops.years = py.float.fromJSON(asJS(this.ops.years) + r.div * s);
    }
    this._has_time = 0;
  },
  __add__: function (other) {
...
function eval_contexts(contexts, evaluation_context) {
  evaluation_context = vectors.utils.extend(
    pyeval.pycontext(),
    evaluation_context || {}
  );
  return contexts.reduce(function (result_context, ctx) {
    // __eval_context evaluations can lead to some of `contexts`'s
    // values being null, skip them as well as empty contexts
    if (vectors.utils.isEmpty(ctx)) {
      return result_context;
    }
    if (vectors.utils.isString(ctx)) {
    ...
    switch (ctx.__ref) {
      ...
      case 'compound_context':
        ...
        evaluated = eval_contexts(
          ctx.__contexts,
          vectors.utils.extend({}, evaluation_context, eval_context)
        );
        break;
    }
    // add newly evaluated context to evaluation context for following
    // siblings
    vectors.utils.extend(evaluation_context, evaluated);
    return vectors.utils.extend(result_context, evaluated);
  }, {});
}
...
function eval_domains(domains, evaluation_context) {
  ...
  domains.forEach(function (domain) {
    if (vectors.utils.isString(domain)) {
    ...
    switch (domain.__ref) {
      case 'domain':
        ...
        result_domain.push.apply(
          result_domain,
          py.eval(domain.__debug, wrap_context(evaluation_context))
        );
        break;
...
function eval_groupbys(contexts, evaluation_context) {
  ...
  contexts.forEach(function (ctx) {
    ...
    if (typeof group === 'string') {
      ...
      throw new Error('Got invalid groupby {{' + JSON.stringify(group) + '}}');
    }
    vectors.utils.extend(evaluation_context, evaluated);
  });
  return result_group;
...
pyeval.pycontext = function pycontext() {
  return vectors.utils.extend(
    {
      datetime: datetime,
...
pyeval.eval_domains_and_contexts = function eval_domains_and_contexts(source) {
  return new Promise(function (resolve, reject) {
    setTimeout(function () {
      var result;
      result = pyeval.sync_eval_domains_and_contexts(source);
      try {
        // result = pyeval.sync_eval_domains_and_contexts(source);
      } catch (e) {
        result = {
...
pyeval.sync_eval_domains_and_contexts = function sync_eval_domains_and_contexts(
  source
) {
  var contexts = [].concat(source.contexts); //[session.user_context] ||
  // see Session.eval_context in Python
  return {
    context: pyeval.eval('contexts', contexts),
    domain: pyeval.eval('domains', source.domains),
    group_by: pyeval.eval('groupbys', source.group_by_seq || []),
  };
};

pyeval.py_eval = function py_eval(expr, context) {
  return py.eval(
    expr,
    vectors.utils.extend({}, context || {}, {
      true: true,
      false: false,
      null: null,
    })
...
pyeval.eval = function (type, object, context) {
  var options = options || {};
  context = vectors.utils.extend(pyeval.pycontext(), context || {});

  //noinspection FallthroughInSwitchStatementJS
  switch (type) {
    case 'context':
    case 'contexts':
      if (type === 'context') object = [object];
      // return eval_contexts((options.no_user_context ? [] : [session.user_context]).concat(object), context);
      //return eval_contexts((options.no_user_context ? [] : []).concat(object), context);
      return eval_contexts(object, context);

    case 'domain':
    case 'domains':
      if (type === 'domain') object = [object];
