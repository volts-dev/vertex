...
export class TreeController extends BasicController {
  /**
  ...
  getActiveDomain() {
    ...
    if (this.$('thead .o_list_record_selector input').prop('checked')) {
      var searchQuery = this._controlPanel
        ? this._controlPanel.getSearchQuery()
        : {};
  ...
  renderButtons($node) {
    if (!this.noLeaf && this.hasButtons) {
      ...
      this._assignCreateKeyboardBehavior(
      ...
      this.$buttons.on(
        ...
        this._onDiscardMousedown.bind(this)
      );
      this.$buttons.on(
        'click',
        '.o_list_button_discard',
  ...
  _addRecord(dataPointId) {
    ...
    return this.renderer
      ...
      .then(function (recordID) {
        ...
        self.renderer
          .updateState(state, { keepWidths: true })
          .then(function () {
   ...
   */
  _assignCreateKeyboardBehavior($createButton) {
    var self = this;
  ...
  _confirmSave(id) {
    var state = this.model.get(this.handle);
    return this.renderer
      .updateState(state, { noRender: true })
   ...
   * @returns {Promise}
   */
  _saveMultipleRecords(recordId, node, changes) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (validRecordIds.length > 0) {
        ...
        new ListConfirmDialog(this, record, dialogChanges, dialogOptions).open({
  ...
  _toggleCreateButton() {
    if (this.$buttons) {
      var state = this.model.get(this.handle);
      var createHidden =
        this.renderer.isEditable() &&
        state.groupedBy.length &&
        state.data.length;
      this.$buttons
        .find('.o_list_button_add')
        .toggleClass('o_hidden', !!createHidden);
    }
  }
  /**
  ...
  _onDiscardMousedown(ev) {
    ...
    window.addEventListener(
      ...
      function (mouseupEvent) {
        ...
        if (
          ev.target !== mouseupEvent.target &&
          preventedEvent.constructor.name === 'OdooEvent'
        ) {
  ...
  _onEditLine(ev) {
    ...
    this.trigger_up('mutexify', {
      action: function () {
        self._setMode('edit', ev.data.recordId).then(ev.data.onSuccess);
      },
    });
  }
  ...
  _onFieldChanged(ev) {
    ...
    if (this.fieldChangedPrevented) {
      ...
      const saveMulti = () => {
        this.multipleRecordsSavingPromise = this._saveMultipleRecords(
          ev.data.dataPointID,
          ev.target.__node,
          ev.data.changes
        );
  ...
  _onToggleGroup(ev) {
    ev.stopPropagation();
    var self = this;
    this.model.toggleGroup(ev.data.group.id).then(function () {
