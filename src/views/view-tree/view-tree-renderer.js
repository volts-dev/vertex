...
export class TreeRenderer extends BasicRenderer {
  /*
    events: {
  ...
  updateState(state, params) {
    ...
    return super.updateState(...argument);
    //return this._super.apply(this, arguments);
  }
   ...
   * @param {Object} column
   */
  _computeColumnAggregates(data, column) {
    ...
    var func = (attrs.sum && "sum") || (attrs.avg && "avg") || (attrs.max && "max") || (attrs.min && "min");
    if (func) {
      var count = 0;
   ...
   * @returns {jQuery|null}
   */
  _findConnectedCell($cell, direction, colIndex) {
   ...
   * @returns {string}
   */
  _getOptionalColumnsStorageKeyParts() {
    var self = this;
    return {
   ...
   */
  _renderAggregateCells(aggregateValues) {
    var self = this;
   ...
   * @returns {jQuery} a <button> element
   */
  _renderGroupButtons(list, group) {
    var self = this;
    var $buttons = $();
    if (list.value) {
   ...
   * @returns {jQueryElement} a <tr> element
   */
  _renderGroupRow(group, groupLevel) {
    var cells = [];
    ...
    var $arrow = $("<span>")
      ...
      .addClass("fa");
    if (group.count > 0) {
      $arrow.toggleClass("fa-caret-right", !group.isOpen).toggleClass("fa-caret-down", group.isOpen);
    }
    $th.prepend($arrow);
    cells.push($th);
   ...
   * @returns {jQueryElement} a <th> element
   */
  _renderOptionalColumnsDropdown() {
    var self = this;
    ...
    this.optionalColumns.forEach(function (col) {
      var txt = (col.attrs.string || self.state.fields[col.attrs.name].string) + (config.isDebug() ? " (" + col.attrs.name + ")" : "");
   ...
   */
  _renderSelector(tag, disableInput) {
   ...
   */
  _setDecorationClasses(record, $tr) {
   ...
   */
  _onGroupButtonClicked(record, node, ev) {
    ev.stopPropagation();
   ...
   * @private
   */
  _onToggleOptionalColumnDropdown(ev) {
