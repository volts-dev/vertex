...
export class TreeModel extends BasicModel {
  /**
   * @override
   ...
   */
  get() {
    var result = super.get(...arguments);
   ...
   */
  saveRecords(listDatapointId, referenceRecordId, recordIds, fieldName) {
    var self = this;
    ...
    return this._rpc({
      ...
      .then(function () {
        return Promise.all([self._fetchX2ManysBatched(list), self._fetchReferencesBatched(list)]);
      });
  }
  ...
  _readGroup(list, options) {
    ...
    return super._readGroup(list, options).then(function (result) {
  ...
  _readGroupExtraFields(list) {
    ...
    var groupIds = _.reduce(
      list.data,
      function (groupIds, id) {
        var resId = self.get(id, { raw: true }).res_id;
        if (resId) {
          // the field might be undefined when grouping
          groupIds.push(resId);
        }
        return groupIds;
      },
