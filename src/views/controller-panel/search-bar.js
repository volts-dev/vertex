...
import { SearchFacet } from './search-facet.js';
...
export class SearchBar extends Element {
  ...
  connetedCallback() {
    ...
    var self = this;
    var defs = [super.connetedCallback(...arguments)];
  ...
  _focusFollowing() {
    ...
    if (focusedIndex === this.searchFacets.length - 1) {
      ...
      $toFocus =
        this.searchFacets.length && this.searchFacets[focusedIndex + 1].$el;
    }
  ...
  _getAutoCompleteSources(req, callback) {
    ...
    Promise.all(defs).then(function (result) {
      var resultCleaned = _(result).chain().compact().flatten(true).value();
  ...
  _setupAutoCompletionWidgets() {
    ...
    _.each(this.filterFields, function (filter) {
      ...
      if (Obj) {
        self.autoCompleteSources.push(
          new Obj(self, filter, field, self.context)
        );
      }
    });
  }
