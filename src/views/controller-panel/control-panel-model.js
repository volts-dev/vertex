import { AbstractModel } from '@/views/abstract_model';
...
function isEqualTo(array1) {
  if (array1.length === 1) {
    ...
    return array2 =>
      array2.length === 2 && array2[0] === array1[0] && array2[1] === array1[1];
  }
}
...
class ControlPanelModel extends AbstractModel {
  ...
  constructor(parent, params) {
    ...
    COMPARISON_TIME_RANGE_OPTIONS = COMPARISON_TIME_RANGE_OPTIONS.map(function (
      option
    ) {
      return _.extend({}, option, {
        description: option.description.toString(),
   ...
   */
  activateTimeRange(filterId, timeRangeId, comparisonTimeRangeId) {
   ...
   */
  createNewFavorite(newFavorite) {
    return this._saveQuery(
      _.pick(newFavorite, ['description', 'isDefault', 'isShared', 'type'])
    ).then(function () {
  ...
  getQuery() {
    ...
    if (context.timeRangeMenuData) {
      if (typeof context.timeRangeMenuData.timeRange === 'string') {
        context.timeRangeMenuData.timeRange = pyUtils.eval(
          'domain',
          context.timeRangeMenuData.timeRange
        );
      }
      if (typeof context.timeRangeMenuData.comparisonTimeRange === 'string') {
        context.timeRangeMenuData.comparisonTimeRange = pyUtils.eval(
          'domain',
    ...
    if (results.error) {
      throw new Error(
        _.str.sprintf(
          _t('Failed to evaluate search criterions') + ': \n%s',
   ...
   */
  toggleAutoCompletionFilter(params) {
  ...
  toggleFilterWithOptions(filterId, optionId) {
    ...
    const defaultYearId = optionId => {
      const year = filter.options.find(o => o.optionId === optionId)
        .defaultYear;
    ...
    if (filter.type === 'filter') {
      ...
      if (index === -1) {
        group.activeFilterIds = this._insert(
          group.activeFilterIds,
          combinationId
        );
        filter.currentOptionIds.add(optionId);
   ...
   * @returns {string}
   */
  _computeDateFilterDomain(filter) {
    const domains = [];
    const p = _.partition(
   ...
   */
  _generateNewGroupNumber() {
    var self = this;
    var groupNumber =
      1 +
      Object.keys(this.filters).reduce(function (max, filterId) {
        var filter = self.filters[filterId];
  ...
  _getFacets() {
    ...
    return this.query.map(groupId => {
      ...
      if (group.type === 'groupBy') {
        ...
        facet.filters = _.compact(
          group.activeFilterIds.map(id => self.filters[id[0]])
        );
      }
      return facet;
    });
  ...
  _getGroupBy() {
    var self = this;
    var groupBys = this.query.reduce(function (acc, groupId) {
      var group = self.groups[groupId];
  ...
  _getGroupGroupBys(group) {
    var self = this;
    var groupBys = group.activeFilterIds.reduce(function (acc, filterId) {
  ...
  _getQueryContext() {
    var self = this;
    var contexts = this.query.reduce(function (acc, groupId) {
      var group = self.groups[groupId];
   ...
   * @returns {Object}
   */
  _getTimeRangeMenuData(evaluation) {
    var context = {};
    ...
    if (groupOfTimeRanges && groupOfTimeRanges.activeFilterIds.length) {
      ...
      var timeRangeDescription = _.find(filter.timeRangeOptions, function (
        option
      ) {
      ...
      if (filter.comparisonTimeRangeId) {
        ...
        comparisonTimeRangeDescription = _.find(
      ...
      if (evaluation) {
        ...
        comparisonTimeRange = Domain.prototype.stringToArray(
          comparisonTimeRange
        );
      }
      context = {
  ...
  _loadFavorites() {
    var self = this;
    var def = this.loadFilters(this.modelName, this.actionId).then(function (
      favorites
    ) {
      if (favorites.length) {
  ...
  _saveQuery(favorite) {
    ...
    var context = pyUtils.eval(
      'contexts',
      [userContext, controllerQueryParams.context, timeRangeMenuInfo].concat(
        queryContext
      )
    );
