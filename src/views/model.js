...
export class BasicModel extends Model {
   ...
   */
  addDefaultRecord(listID, options) {
    var self = this;
    var list = this.localData[listID];
    var context = _.extend(
      {},
      this._getDefaultContext(list),
   ...
   */
  addFieldsInfo(recordID, viewInfo) {
   ...
   * @returns {Promise}
   */
  applyDefaultValues(recordID, values, options) {
    options = options || {};
    var record = this.localData[recordID];
   ...
   */
  applyRawChanges(recordID, viewType) {
    var record = this.localData[recordID];
    return this._applyOnChange(record._rawChanges, record, viewType).then(
      function () {
        return record.id;
      }
    );
   ...
   * @returns {Promise}
   */
  deleteRecords(recordIds, modelName) {
    var self = this;
  ...
  load(params) {
    params.type =
      params.type || (params.res_id !== undefined ? 'record' : 'list');
   ...
   */
  makeRecord(model, fields, fieldInfo) {
    var self = this;
    var defs = [];
    var record_fields = {};
    _.each(fields, function (field) {
      record_fields[field.name] = _.pick(
        field,
        'type',
        'relation',
        'domain',
    ...
    _.each(fields, function (field) {
      ...
      if (field.type === 'many2one') {
        if (field.value) {
          ...
          var display_name = _.isArray(field.value)
            ? field.value[1]
            : undefined;
   ...
   */
  notifyChanges(record_id, changes, options) {
    return this.mutex.exec(
      this._applyChange.bind(this, record_id, changes, options)
    );
  }
  /**
   ...
   */
  resequence(modelName, resIDs, parentID, options) {
    options = options || {};
    if (resIDs.length <= 1) {
    ...
    return this.mutex.exec(function () {
      ...
      var prom = new Promise(function (resolve, reject) {
        ...
        if (method === 'create' || changedFields.length) {
          var args =
            method === 'write' ? [[record.data.id], changes] : [changes];
          self
            ._rpc({
              model: record.model,
              method: method,
   ...
   */
  toggleActive(recordIDs, parentID) {
    var self = this;
   ...
   * @returns {Promise<string>} resolves to the parent id
   */
  actionArchive(recordIDs, parentID) {
    var self = this;
    var parent = this.localData[parentID];
    var resIDs = _.map(recordIDs, function (recordID) {
      return self.localData[recordID].res_id;
   ...
   * @returns {Promise<string>} resolves to the parent id
   */
  actionUnarchive(recordIDs, parentID) {
    var self = this;
    var parent = this.localData[parentID];
    var resIDs = _.map(recordIDs, function (recordID) {
      return self.localData[recordID].res_id;
   ...
   */
  _addX2ManyDefaultRecord(list, options) {
    var self = this;
    var position = (options && options.position) || 'top';
    var params = {
      fields: list.fields,
    ...
    return Promise.all(defs).then(function () {
      ...
      return new Promise(function (resolve, reject) {
        if (onChangeFields.length) {
          self
            ._performOnChange(record, onChangeFields, options.viewType)
   ...
   * @returns {Promise}
   */
  _applyX2OneChange(record, fieldName, data) {
    var self = this;
    if (!data || !data.id) {
   ...
   * @returns {Promise}
   */
  _applyOnChange(values, record, viewType) {
    var self = this;
    var defs = [];
    var rec;
    ...
    _.each(values, function (val, name) {
      ...
      var oldValue =
        name in record._changes ? record._changes[name] : record.data[name];
      var id;
      if (field.type === 'many2one') {
        id = false;
        ...
        if (val) {
          ...
          if (
            !oldValue ||
            self.localData[oldValue].res_id !== resID ||
   ...
   * @returns {Promise}
   */
  _applyX2ManyChange(record, fieldName, command, options) {
    ...
    var self = this;
    var localID =
      (record._changes && record._changes[fieldName]) || record.data[fieldName];
    ...
    switch (command.operation) {
      ...
      case 'CREATE':
        ...
        def = this._addX2ManyDefaultRecord(list, createOptions).then(function (
          ids
        ) {
      ...
      case 'DELETE_ALL':
        ...
        return this._applyX2ManyChange(
          record,
          fieldName,
          {
            operation: 'DELETE',
      ...
      case 'REPLACE_WITH':
        ...
        if (newIds.length) {
          ...
          addDef = this._applyX2ManyChange(
            record,
            fieldName,
            {
              operation: 'ADD_M2M',
        ...
        if (removedIds.length) {
          ...
          removedDef = this._applyX2ManyChange(
            record,
            fieldName,
            {
              operation: 'DELETE',
      ...
      case 'MULTI':
        ...
        _.each(command.commands, function (innerCommand) {
          defs.push(
            self._applyX2ManyChange(record, fieldName, innerCommand, options)
          );
        });
        break;
    }
   ...
   *   applied
   */
  _applyX2ManyOperations(list, options) {
    if (!list.static) {
    ...
    _.each(changes, function (change) {
      ...
      switch (change.operation) {
        case 'ADD':
          ...
          if (
            change.position === 'top' &&
            (options ? options.position !== 'bottom' : true)
          ) {
   ...
   *   be applied
   */
  _buildOnchangeSpecs(record, viewType) {
   ...
   */
  _computeOverrideDefaultFields(listID, position) {
    var list = this.localData[listID];
    var handleField;
    ...
    if (position === 'top') {
      ...
      list.count <=
      list.offset + list.limit - (list.tempLimitIncrement || 0)
    ) {
      handleFieldValue++;
    }
    return {
  ...
  _fetchMany2OneGroup(groups) {
    ...
    return this._rpc({
      ...
      _.each(groups, function (obj) {
        var nameGet = _.find(name_gets, function (n) {
   ...
   * @returns {Promise}
   */
  _fetchReference(record, fieldName) {
    var self = this;
    var def;
    var value =
      (record._changes && record._changes[fieldName]) || record.data[fieldName];
   ...
   * @returns {Promise}
   */
  _fetchReferenceData(datapoints, model, fieldName) {
    var self = this;
    var ids = _.map(Object.keys(datapoints), function (id) {
      return parseInt(id);
    });
   ...
   * @returns {Promise}
   */
  _fetchReferences(record, options) {
    var self = this;
    var defs = [];
   ...
   * @returns {Promise}
   */
  _fetchReferenceBatched(list, fieldName) {
    var self = this;
    list = this._applyX2ManyOperations(list);
    this._sortList(list);
   ...
   * @returns {Promise}
   */
  _fetchReferenceSingleBatch(list, fieldName) {
    var self = this;
   ...
   * @returns {Promise}
   */
  _fetchReferencesSingleBatch(list) {
    var defs = [];
    var fieldNames = list.getFieldNames();
   ...
   * @returns {Promise}
   */
  _fetchRelatedData(list, toFetch, fieldName) {
    var self = this;
  ...
  _fetchRelationalData(record) {
    ...
    _.each(record.getFieldNames(), function (name) {
      ...
      if (
        field.type === 'many2one' &&
        !record.fieldsInfo[record.viewType][name].__no_fetch
      ) {
        var localId =
          (record._changes && record._changes[name]) || record.data[name];
   ...
   */
  _fetchSpecialData(record, options) {
    var self = this;
    ...
    return Promise.all(
      _.map(fieldNames, function (name) {
        ...
        if (Widget && Widget.prototype.specialData) {
          return self[Widget.prototype.specialData](
            record,
            name,
            fieldInfo
          ).then(function (data) {
   ...
   *          the undefined value.
   */
  _fetchSpecialMany2ones(record, fieldName, fieldInfo, fieldsToRead) {
    var field = record.fields[fieldName];
    if (field.type !== 'many2one') {
      return Promise.resolve();
    }
   ...
   * @returns {Promise}
   */
  _fetchSpecialReference(record, fieldName) {
    var def;
    var field = record.fields[fieldName];
    if (field.type === 'char') {
   ...
   *          the undefined value.
   */
  _fetchSpecialStatus(record, fieldName, fieldInfo) {
    var foldField = fieldInfo.options.fold_field;
    var fieldsToRead = foldField ? [foldField] : [];
    return this._fetchSpecialMany2ones(
      record,
      fieldName,
      fieldInfo,
      fieldsToRead
   ...
   *          the undefined value.
   */
  _fetchSpecialDomain(record, fieldName, fieldInfo) {
    var self = this;
    var context = record.getContext({ fieldName: fieldName });
   ...
   */
  _fetchUngroupedList(list, options) {
   ...
   * @returns {Promise}
   */
  _fetchX2ManyBatched(list, fieldName) {
    list = this._applyX2ManyOperations(list);
    this._sortList(list);
   ...
   * @returns {Promise}
   */
  _fetchX2ManySingleBatch(list, fieldName) {
    var self = this;
    var toFetch = {};
    _.each(list.data, function (groupIndex) {
      var group = self.localData[groupIndex];
   ...
   */
  _generateChanges(record, options) {
    options = options || {};
    var viewType = options.viewType || record.viewType;
    var changes;
    ...
    for (var fieldName in record.fields) {
      ...
      if (type === 'one2many' || type === 'many2many') {
        if (commands[fieldName] && commands[fieldName].length) {
          // replace localId by commands
          changes[fieldName] = commands[fieldName];
        } else {
          // no command -> no change for that field
          delete changes[fieldName];
        }
   ...
   */
  _generateOnChangeData(record, options) {
   ...
   */
  _generateX2ManyCommands(record, options) {
    var self = this;
    options = options || {};
    ...
    for (var fieldName in fields) {
      ...
      if (type === 'many2many' || type === 'one2many') {
        ...
        if (type === 'many2many' || list._forceM2MLink) {
          ...
          var realIDs = _.difference(
            list.res_ids,
            _.pluck(relRecordCreated, 'res_id')
          );
          ...
          _.each(relRecordCreated, function (relRecord) {
            ...
            commands[fieldName].push(
              x2ManyCommands.create(relRecord.ref, changes)
            );
          });
          ...
          for (var i = 0; i < list.res_ids.length; i++) {
            if (_.contains(keptIds, list.res_ids[i])) {
              ...
              relRecord = _.findWhere(relRecordUpdated, {
                res_id: list.res_ids[i],
              });
              changes = relRecord
                ? this._generateChanges(relRecord, options)
                : {};
              if (!_.isEmpty(changes)) {
              ...
              relRecord = _.findWhere(relRecordAdded, {
                res_id: list.res_ids[i],
              });
              if (!relRecord) {
                commands[fieldName].push(
                  x2ManyCommands.link_to(list.res_ids[i])
                );
                continue;
              }
              changes = this._generateChanges(
                relRecord,
                _.extend({}, options, { changesOnly: true })
              );
              if (!this.isNew(relRecord.id)) {
                // the subrecord already exists in db
                commands[fieldName].push(
                  x2ManyCommands.link_to(relRecord.res_id)
                );
                delete changes.id;
                if (!_.isEmpty(changes)) {
                  commands[fieldName].push(
                    x2ManyCommands.update(relRecord.res_id, changes)
                  );
                }
              } else {
   ...
   */
  _getDataToFetch(list, fieldName) {
    var self = this;
    var field = list.fields[fieldName];
    var fieldInfo = list.fieldsInfo[list.viewType][fieldName];
    var view = fieldInfo.views && fieldInfo.views[fieldInfo.mode];
    ...
    for (let i = 0; i < list.groupedBy.length; i++) {
      dataPointIds = dataPointIds.reduce(
        (acc, groupId) => acc.concat(this.localData[groupId].data),
        []
      );
    }
   ...
   */
  _getDataToFetchByModel(list, fieldName) {
    var self = this;
    var toFetch = {};
   ...
   * @returns {Object}
   */
  _getEvalContext(element, forDomain) {
    var evalContext =
      element.type === 'record'
        ? this._getRecordEvalContext(element, forDomain)
        : {};
    ...
    if (session.user_context.allowed_company_ids) {
      ...
      var current_company = session.user_companies
   ...
   */
  _getFieldNames(element, options) {
   ...
   */
  _getMany2OneFieldNames(datapoint) {
    var many2ones = [];
  ...
  _getRecordEvalContext(record, forDomain) {
    ...
    for (var fieldName in context) {
      ...
      if (field.type === 'one2many' || field.type === 'many2many') {
        var ids;
        if (!context[fieldName] || _.isArray(context[fieldName])) {
          // no dataPoint created yet
          ids = context[fieldName] ? context[fieldName].slice(0) : [];
        } else {
          relDataPoint = this._applyX2ManyOperations(
            this.localData[context[fieldName]]
          );
   ...
   * @returns {boolean}
   */
  _isFieldProtected(record, fieldName, viewType) {
    var fieldInfo =
      record.fieldsInfo &&
      record.fieldsInfo[viewType || record.viewType][fieldName];
    if (fieldInfo) {
      var rawModifiers = fieldInfo.modifiers || {};
      var modifiers = this._evalModifiers(
        record,
        _.pick(rawModifiers, 'readonly')
      );
  ...
  _isX2ManyValid(id) {
    ...
    _.each(element._changes, function (command) {
      ...
      _.each(element.getFieldNames(), function (fieldName) {
        ...
        var rawModifiers = fieldInfo.modifiers || {};
        var modifiers = self._evalModifiers(
          record,
          _.pick(rawModifiers, 'required')
        );
        if (
          modifiers.required &&
          !self._isFieldSet(recordData[fieldName], field.type)
        ) {
          isValid = false;
        }
      });
  ...
  _makeDataPoint(params) {
    ...
    var dataPoint = {
      ...
      offset:
        params.offset || (type === 'record' ? _.indexOf(res_ids, res_id) : 0),
   ...
   */
  _makeDefaultRecord(modelName, params) {
    var self = this;
      ...
      return self
        ...
        .then(function () {
          var def = new Promise(function (resolve, reject) {
            ...
            self
              ._performOnChange(record, fields_key)
              .then(always)
              .guardedCatch(always);
          });
          return def;
   ...
   */
  _parseServerData(fieldNames, element, data) {
    var self = this;
    _.each(fieldNames, function (fieldName) {
      var field = element.fields[fieldName];
   ...
   * @returns {Promise}
   */
  _performOnChange(record, fields, viewType) {
    var self = this;
    ...
    Promise.resolve().then(function () {
      ...
      for (var i = 0; i < batchedRPCsRequests.length; i++) {
        ...
        if (!batches[key]) {
          ...
          batches[key].fieldNames = _.uniq(
            batches[key].fieldNames.concat(request.fieldNames)
          );
      ...
      for (key in batches) {
        ...
        self
          ...
          .then(onSuccess.bind(null, batch))
          .guardedCatch(onFailure.bind(null, batch));
      }
    });
          ...
          defs.push(
            self
              ._rpc({
                ...
                context: self._getContext(record, {
                  fieldName: name,
                  viewType: viewType,
                }),
              })
   ...
   * @returns {Promise}
   */
  _processX2ManyCommands(record, fieldName, commands, options) {
    var self = this;
    options = options || {};
    var defs = [];
    var field = record.fields[fieldName];
    var fieldInfo =
      record.fieldsInfo[options.viewType || record.viewType][fieldName] || {};
    var view = fieldInfo.views && fieldInfo.views[fieldInfo.mode];
    var fieldsInfo = view ? view.fieldsInfo : fieldInfo.fieldsInfo;
    var fields = view ? view.fields : fieldInfo.relatedFields;
    var viewType = view ? view.type : fieldInfo.viewType;
        ...
        for (var fieldName in r._changes) {
          ...
          if (isFieldInView) {
            ...
            if (fieldType === 'many2one') {
              ...
              r._changes[fieldName] = self._parseServerValue(
                field,
                r._changes[fieldName]
              );
            }
          }
        }
   ...
   */
  _readMissingFields(list, resIDs, fieldNames) {
    var self = this;
  ...
  _readGroup(list, options) {
    ...
    var orderedBy = _.filter(list.orderedBy, function (order) {
      return (
        order.name === rawGroupBy ||
        list.fields[order.name].group_operator !== undefined
      );
    });
    ...
    return this._rpc({
      ...
      _.each(groups, function (group) {
        ...
        if (oldGroup) {
          ...
          if (
            options.onlyGroups ||
            (oldGroup.isOpen && newGroup.groupedBy.length)
          ) {
          ...
          !newGroup.openGroupByDefault ||
          openGroupCount >= openGroupsLimit
        ) {
      ...
      if (options.keepEmptyGroups) {
        ...
        var emptyGroupsIDs = _.difference(
          _.pluck(previousGroups, 'id'),
          list.data
        );
  ...
  _readUngroupedList(list) {
    ...
    if (list.res_ids.length > list.limit && list.orderedBy.length) {
      if (!list.orderedResIDs) {
        ...
        def = this._readMissingFields(
          list,
          _.filter(list.res_ids, _.isNumber),
          fieldNames
        );
      }
      def.then(function () {
    ...
    return def.then(function () {
      ...
      var upperBound = list.limit
        ? Math.min(list.offset + list.limit, currentCount)
        : currentCount;
      var fieldNames = list.getFieldNames();
   ...
   */
  _saveSpecialDataCache(record, fieldName, value) {
  ...
  _sortList(list) {
    ...
    if (list.orderedResIDs) {
      ...
      utils.stableSort(list.res_ids, function compareResIdIndexes(
        resId1,
        resId2
      ) {
      ...
      var compareRecords = function (resId1, resId2, level) {
        ...
        if (list.fields[order.name].type === 'many2one') {
          orderData1 = orderData1
            ? self.localData[orderData1].data.display_name
            : '';
          orderData2 = orderData2
            ? self.localData[orderData2].data.display_name
            : '';
        }
   ...
   */
  _updateParentResIDs(element) {
    var self = this;
    if (element.parentID) {
      var parent = this.localData[element.parentID];
      parent.res_ids = _.flatten(
        _.map(parent.data, function (dataPointID) {
   ...
   */
  _updateRecordsData(records, fieldName, values) {
