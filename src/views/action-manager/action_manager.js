...
//import '../action-viewer/action-viewer.js';

var _t = Translation._t;

// action 监听触发所有页面消息
export class ActionManager extends Element {
  /*render() {
    return html`
      <!--controller-->
      <!--action-controller id="control_panel" class="paper-header" .app="${this}" .path="${this
        .path}" .lastUrl="${this.lastUrl}"></action-controller!-->
      <!--viewer-->
      <!--action-viewer id="view_manager" class="content" .app="${this}" .path="${this
        .path}" .lastUrl="${this.lastUrl}"></action-viewer!-->
    `;
  }
*/
...
*/
  ...
  constructor() {
    ...
    this.controllers = {}; // 存储 controllers 集
  ...
  connectedCallback() {
    ...
    if (currentController) {
      currentController.widget.connectedCallback();
    }
  }
  ...
  disconnectedCallback() {
    ...
    var currentController = this.getCurrentController();
    if (currentController) {
      currentController.widget.disconnectedCallback();
    }
  }
  ...
  getCurrentController() {
    ...
    var currentController = currentControllerID
      ? this.controllers[currentControllerID]
      : null;

    var action = currentController && this.actions[currentController.actionID];
    if (
      action &&
      action.type === 'sys.action.window' &&
      !currentController.widget
    ) {
      var lastControllerID = this.controllerStack.pop();
      //var currentControllerID = _.last(this.controllerStack);
      //var currentController = currentControllerID ? this.controllers[currentControllerID] : null;
  ...
  LoadState(state) {
    ...
    if (state.action) {
      var currentController = this.getCurrentController();
      var currentAction =
        currentController && this.actions[currentController.actionID];

      // 检测是否已经使用相同控制器
      if (
        currentAction &&
        currentAction.id === state.action &&
        currentAction.type === 'sys.action.window'
      ) {
      ...
      } else if (!service.action_registry.contains(state.action)) {
  ...
  /**
   * This is the entry point to execute actions, given as an ID in
  ...
  doAction(action, options) {
    ...
    var def;
    if (_.isString(action) && service.action_registry.contains(action)) {
    ...
    return this.dp.add(Promise.resolve(def)).then(function () {
      ...
      options.clear_breadcrumbs =
        action.target === 'main' || options.clear_breadcrumbs;
      ...
      controllerDef = controllerDef.then(
        function (controller) {
          if (!controller.widget) {
            ...
            return Promise.resolve(controller.widget.willRestore()).then(
              function () {
  ...
  _createViewController(action, viewType, viewOptions, options) {
    ...
    if (!options.lazy) {
      ...
      var rejection;
      // 创建视图
      var view = new viewDescr.Element(viewDescr.fieldsView, viewOptions); // new viewDescr(viewDescr.fieldsView, viewOptions);
      var def = new Promise(function (resolve, reject) {
        rejection = reject;

        // 从View中生成Controller控制板
        // 并保存于action.controllers
        view
          .getController(self)
   ...
   */
  // 插入控制器节点
  _appendController(controller) {
    this.shadowRoot.append(controller.widget);
    // TODO
    /*
   ...
   */
  _executeWindowAction(action, options) {
    var self = this;
    return this.dp.add(this._loadViews(action)).then(function (fieldsViews) {
      ...
      var curView =
        options.viewType && _.findWhere(views, { type: options.viewType });
      var lazyView;
      if (curView) {
      ...
      if (Config.device.isMobile) {
        ...
        if (lazyView && !lazyView.isMobileFriendly) {
          lazyView =
            self._findMobileView(views, lazyView.multiRecord) || lazyView;
        }
      }
      ...
      if (lazyView) {
        ...
        lazyViewDef = self
          ._createViewController(action, lazyView.type, {}, { lazy: true })
      ...
      return self.dp
        .add(Promise.resolve(lazyViewDef))
        .then(function () {
          var viewOptions = {
            controllerState: options.controllerState,
            currentId: options.resID,
          };
          var curViewDef = self._createViewController(
            action,
            curView.type,
            viewOptions,
            {
  ...
  _generateActionViews(action, fieldsViews) {
    ...
    _.each(action.views, function (view) {
      ...
      var parsedXML = new DOMParser().parseFromString(
        fieldsView.arch,
        'text/xml'
      );
      var key = parsedXML.documentElement.getAttribute('js_class');
      var key = key || parsedXML.documentElement.tagName || viewType;
      var View = view_registry.get(key);
      if (View) {
        views.push({
        ...
        console.log(
          "View type '" + viewType + "' is not present in the view registry."
        );
      }
    });
  ...
  _executeClientAction(action, options) {
    var self = this;
    var ClientAction = service.action_registry.get(action.tag);
    if (!ClientAction) {
    ...
    if (!(ClientAction.prototype instanceof AbstractAction)) {
      console.warn(
        'The client action ' +
          action.tag +
          ' should be an instance of AbstractAction!'
      );
    }
    ...
    options.breadcrumbs = this._getBreadcrumbs(
      this.controllerStack.slice(0, index)
    );
   ...
   */
  _executeServerAction(action, options) {
    var self = this;
   ...
   * @returns {Object}
   */
  _getControllerState(controllerID) {
    ...
    if (action.context) {
      ...
      if (
        active_ids &&
        !(active_ids.length === 1 && active_ids[0] === active_id)
      ) {
  ...
  _handleAction(action, options) {
    ...
    switch (action.type) {
      ...
      default:
        console.error(
          "The ActionManager can't handle actions of type " + action.type,
          action
        );
        return Promise.reject();
    }
...
customElements.define('action-manager', ActionManager);
