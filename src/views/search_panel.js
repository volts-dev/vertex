...
export class SearchPanel extends store.Element {
  /**
   * @override
   * @param {Object} params
  ...
  willStart() {
    ...
    if (this.initialState) {
      ...
      loadCategoriesProm = this._fetchCategories().then(function () {
        return self
          ._fetchFilters()
          .then(self._applyDefaultFilterValues.bind(self));
      });
    }
    return Promise.all([
      loadCategoriesProm,
      ...
      arch.children.forEach(function (node) {
        if (node.tag === 'searchpanel') {
          ...
          if (viewTypes.indexOf(viewType) !== -1) {
            searchPanelSections = _processSearchPanelNode(
              node,
              viewInfo.fields
            );
          }
   ...
   */
  update(params) {
    const currentDomain = JSON.stringify([
      ...this.searchDomain,
      ...this.viewDomain,
    ]);
    const newDomain = JSON.stringify([
      ...params.searchDomain,
      ...params.viewDomain,
    ]);
    var filtersProm;
  ...
  _renderFilter(filter) {
    ...
    Object.keys(filter.groups || {}).forEach(function (groupId) {
      ...
      if (groupId !== 'false' && state === 'indeterminate') {
        $filter
          .find(
            '.o_search_panel_filter_group[data-group-id=' + groupId + '] input'
          )
  ...
  _updateFilterGroupState(group) {
    var valuePartition = _.partition(Object.keys(group.values), function (
      valueId
    ) {
