...
// 数据的缓存和更新
export class TDataManager {
  constructor() {
    this._init_cache();
    //OSV.bus.on("clear_cache", this, this.invalidate.bind(this));
  }
   ...
   */
  invalidate() {
    store.session.invalidateCacheKey('load_menus');
    this._init_cache();
  }
  ...
  load_action(action_id, additional_context) {
    ...
    return this._cache.actions[key].then(function (action) {
      return action; //$.extend(true, {}, action);
    });
  }
  ...
  load_views(params, options) {
    ...
    if (Config.isDebug('assets') || !this._cache.views[key]) {
      ...
      this._cache.views[key] = Ajax.query({
        //args: [],
        //kwargs: {},
        params: { views: views_descr, options: options, context: context },
        model: model,
        method: 'load_views',
  ...
  load_filters(params) {
    ...
    if (Config.isDebug('assets') || !this._cache.filters[key]) {
      this._cache.filters[key] = Ajax.query({
  ...
  create_filter(filter) {
    ...
    return Ajax.query({
      ...
      var key = [filter.model_id, filter.action_id || false].join(',');
   ...
   */
  _invalidate(cache, key) {
    delete cache[key];
  }
}

export var DataManager = new TDataManager();
