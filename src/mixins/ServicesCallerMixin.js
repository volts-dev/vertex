 ...
 * @name ServicesMixin
 */
export function ServicesCallerMixin(superClass) {
  return class extends superClass {
     ...
     * @returns {Promise}
     */
    _loadLibs(libs, context, tplRoute) {
      return this.call(
        'ajax',
        'loadLibs',
        libs,
        context,
        tplRoute || this._loadLibsTplRoute
      );
    }
     ...
     */
    _rpc(params, options) {
      var query = Ajax.buildQuery(params);
      var prom = this.call(
        'ajax',
        'rpc',
        query.route,
    ...
    loadFieldView(dataset, view_id, view_type, options) {
    ...
    loadViews(modelName, context, views, options) {
      var self = this;
