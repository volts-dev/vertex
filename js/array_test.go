...
func TestNewFromJSObject(t *testing.T) {
	...
	Eval("customarray=new Array(1,2,5)")
	if obj = Global().Get("customarray"); test.AssertErr(t, obj.Error()) {
...
func TestIndexOf(t *testing.T) {
	...
	if a, err = New(goArray...); test.AssertErr(t, err) {
		...
		if index, err := a.IndexOf(b); test.AssertErr(t, err) {
			...
			if index != 2 {
				t.Errorf("index must be 2 have %d when searching %s", index, js.ValueToString(obj))
			}
