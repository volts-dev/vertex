...
func GetInterface() js.Value {
	...
	singleton.Do(func() {
		...
		if htmlformelementinterface = js.Global().Get("HTMLFormElement"); htmlformelementinterface.Error() != nil {
			htmlformelementinterface = js.Undefined()
		}
		js.Register(htmlformelementinterface, func(v js.Value) (interface{}, error) {
...
func (h HtmlFormElement) RequestSubmit(elem ...js.Object) error {

	var arrayJS []interface{}

	if len(elem) > 0 {
		arrayJS = append(arrayJS, elem[0].GetObjectValue())
	}

	err := h.Call("requestSubmit", arrayJS...).Error()
	return err
}

func (h HtmlFormElement) Reset() error {
	err := h.Call("reset").Error()
	return err
}

func (h HtmlFormElement) Submit() error {
	err := h.Call("submit").Error()
	return err
}
