...
func TestAll(t *testing.T) {
	...
	if allp, err := All(p1, p2); test.AssertErr(t, err) {
	...
	if allp, err := All(p3, p4); test.AssertErr(t, err) {
...
func TestAllSettled(t *testing.T) {
	...
	if allp, err := AllSettled(p1, p2); test.AssertErr(t, err) {
			...
			if a, ok := i.(array.ArrayFrom); ok {
				...
				if it, err := a.Array_().Entries(); test.AssertErr(t, err) {
					...
					for index, value, err := it.Next(); err == nil; index, value, err = it.Next() {
						...
						if obj, ok := value.(js.ObjectFrom); ok {
							if i, ok := index.(int); ok {
								switch i {
								case 0:
									if obj.BaseObject_().GetAttributeString_("status") != "fulfilled" {
										t.Errorf("Status invalid %s", obj.BaseObject_().GetAttributeString_("status"))
									}
									if obj.BaseObject_().GetAttributeString_("value") != "Hello" {
										t.Errorf("Value invalid %s", obj.BaseObject_().GetAttributeString_("value"))
									}
								case 1:
									if obj.BaseObject_().GetAttributeString_("status") != "rejected" {
										t.Errorf("Status invalid %s", obj.BaseObject_().GetAttributeString_("status"))
									}
									if obj.BaseObject_().GetAttributeString_("reason") != "unknown error" {
										t.Errorf("Reason invalid %s", obj.BaseObject_().GetAttributeString_("reason"))
									}
								}
...
func TestAny(t *testing.T) {
	...
	if anyp, err := Any(p1w, p2); test.AssertErr(t, err) {
...
func TestRace(t *testing.T) {
	...
	if anyp, err := Race(p1w, p2w); test.AssertErr(t, err) {
...
func TestFinally(t *testing.T) {
	...
	if fin, err := All(wait100, wait500); test.AssertErr(t, err) {
...
func TestReject(t *testing.T) {
	...
	if p, err := Reject(errors.New("Failed")); test.AssertErr(t, err) {
...
func TestResolve(t *testing.T) {
	...
	if p, err := Resolve(100); test.AssertErr(t, err) {
