...
func TestNew(t *testing.T) {
	...
	if doc, err := document.New(); test.AssertErr(t, err) {
		if ta, err := New(doc); test.AssertErr(t, err) {
			test.AssertExpect(t, "HTMLTextAreaElement", ta.ConstructName_())
		}
...
func TestNewFromJSObject(t *testing.T) {
	...
	if obj := js.Global().Get("ta"); test.AssertErr(t, obj.Error()) {

		if ta, err := NewFromJSObject(obj); test.AssertErr(t, err) {
...
func TestSetRangeText(t *testing.T) {
	if doc, err := document.New(); test.AssertErr(t, err) {
		if ta, err := New(doc); test.AssertErr(t, err) {
			...
			test.AssertErr(t, ta.SetRangeText("big", 6, 10))
			v, _ := ta.Value()
			test.AssertExpect(t, "hello big world", v)
		}
